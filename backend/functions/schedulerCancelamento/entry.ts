...
Deno.serve(async (req) => {
  ...
  let user = null;
  try { user = await client.auth.me(); } catch {}
  ...
  const solicitacoes = await client.asServiceRole.entities.RetentionRequest.filter({
  ...
  for (const sol of solicitacoes) {
    ...
    if (res.ok) {
      ...
      await client.asServiceRole.entities.RetentionRequest.update(sol.id, {
        status_processo: 'processo_finalizado',
      });

      // Log
      await client.asServiceRole.entities.Comunicacao.create({
        request_id: sol.id,
