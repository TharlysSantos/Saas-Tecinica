...
export default function Requests() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.RetentionRequest.list("-created_date", 100),
  });

  const { data: allActions = [] } = useQuery({
    queryKey: ["all-request-actions"],
    queryFn: () => api.entities.RequestAction.list("-created_date", 500),
  });

  const { data: cancellationReasons = [] } = useQuery({
    queryKey: ["cancellation-reasons"],
    queryFn: () => api.entities.CancellationReason.list(),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => api.entities.RetentionRequest.delete(id),
    onSuccess: () => {
