...
export default function ExAssinantePanel({
  ...
  const handleAvancarReprocessar = async (reqId) => {
    await api.entities.RetentionRequest.update(reqId, {
          ...
          onClick={async () => {
            ...
            try {
              for (const req of filteredRequests) {
                ...
                const d = res.data;
                if (d?.found) {
                  await api.entities.RetentionRequest.update(req.id, {
            ...
            try {
              for (const req of filteredRequests) {
                ...
                if (newStatus !== req.status_store) {
                  await api.entities.RetentionRequest.update(req.id, { status_store: newStatus });
                }
              }
              queryClient.invalidateQueries({ queryKey: ["workflow-requests"] });
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  for (const req of selecionados) {
                    await api.entities.RetentionRequest.update(req.id, { status_processo: "retido" });
                  }
                  queryClient.invalidateQueries({ queryKey: ["workflow-requests"] });
