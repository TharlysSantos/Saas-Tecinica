...
const LOGO_URL = "https://media.api.com/images/public/69aae81f416f0a7fc846ab74/f1cf6be15_01-D-Saaslogotipo20241.png";
...
export default function EnvioDoTermo() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.RetentionRequest.filter({ request_type: "cancelamento" }, "-created_date", 300),
  });
  ...
  const handleOficializar = async () => {
    ...
    for (const req of selecionados) {
      ...
      await api.entities.RetentionRequest.update(req.id, { status_processo: "concluido" });
    }
  ...
  const handleVoltarExecucao = async () => {
    ...
    setProcessing(true);
    for (const req of selecionados) {
      await api.entities.RetentionRequest.update(req.id, { status_processo: "em_execucao" });
    }
