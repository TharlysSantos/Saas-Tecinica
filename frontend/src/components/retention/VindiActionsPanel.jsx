...
export default function VindiActionsPanel({ request, onSaved, onDataImported, currentUser }) {
  ...
  const handleVindiSearch = async () => {
    ...
    const res = await api.functions.invoke('vindiIntegration', { action: 'buscar_por_cnpj', cnpj: cnpjInput });
  ...
  const handleSelectCustomer = async (customerId) => {
    setSearching(true);
    setMultipleSubscriptions(null);
    const res = await api.functions.invoke('vindiIntegration', { action: 'buscar_por_customer_id', customer_id: customerId });
    if (res.data?.multiple) {
      setMultipleSubscriptions({ type: 'subscriptions', items: res.data.subscriptions });
      setMultipleCustomer(res.data.customer);
      setSearching(false);
  ...
  const handleSelectSubscription = async (subId) => {
    setSearching(true);
    setMultipleSubscriptions(null);
    const res = await api.functions.invoke('vindiIntegration', { action: 'buscar_por_id_assinatura', subscription_id: subId });
    await applyVindiData(res.data);
    setSearching(false);
  };
  ...
  const applyVindiData = async (data) => {
    ...
    await api.entities.RetentionRequest.update(request.id, fields);
  ...
  const handleGerarTermo = async () => {
    ...
    const res = await api.functions.invoke("generateTermoPDF", { request_id: request.id });
    if (res.data?.success) {
  ...
  const handleCancelarVindi = async () => {
    ...
    if (res.data?.success) {
      await api.entities.RetentionRequest.update(request.id, {
