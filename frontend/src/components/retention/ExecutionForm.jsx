...
export default function ExecutionForm({ data, onChange, currentUser, onGoBack }) {
  ...
  const buscarProdutos = async () => {
    ...
    try {
      const allConfigs = await api.entities.PlanConfig.list('-created_date', 500);
  ...
  const handleAgendarMulta = async () => {
    ...
    try {
      ...
      if (!customerId && data.id_assinatura) {
        const subRes = await api.functions.invoke('vindiIntegration', {
      ...
      await api.entities.RetentionRequest.update(data.id, {
  ...
  const buscarFaturasDaVindi = async () => {
    ...
    if (!customerId && data.id_assinatura) {
      try {
        ...
        if (customerId) {
          await api.entities.RetentionRequest.update(data.id, { vindi_customer_id: String(customerId) });
        }
      } catch (e) {
