...
export default function RequestSheet({ request, onClose, onSaved }) {
  ...
  const { data: actions = [] } = useQuery({
    ...
    queryFn: () => api.entities.RequestAction.filter({ request_id: request.id }, "-created_date", 100),
    enabled: !!request?.id,
  });

  const { data: comunicacoes = [] } = useQuery({
    queryKey: ["comunicacoes", request?.id],
    queryFn: () => api.entities.Comunicacao.filter({ request_id: request.id }, "-created_date", 50),
    enabled: !!request?.id,
  });

  const addActionMutation = useMutation({
    mutationFn: (payload) => api.entities.RequestAction.create(payload),
    onSuccess: () => {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    await api.entities.RetentionRequest.delete(request.id);
