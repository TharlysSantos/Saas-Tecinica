...
function TermoField({ requestId, possui_termo, link_termo, onSaved }) {
  ...
  const handleUpload = async (e) => {
    ...
    await api.entities.RetentionRequest.update(requestId, { possui_termo: true, link_termo: file_url });
...
export default function TriagemFields({ data, requestId, onSaved }) {
  ...
  const save = async (field, value) => {
    await api.entities.RetentionRequest.update(requestId, { [field]: value });
    onSaved?.({ [field]: value });
  };

  useEffect(() => {
    api.entities.PlanoAuxiliar.filter({ ativo: true }, "nome", 200)
