...
export default function PlanosAuxiliares() {
  ...
  const { data: planos = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.PlanoAuxiliar.list("nome", 200),
  });

  const handleAdd = async () => {
    if (!newNome.trim()) return;
    setSaving(true);
    await api.entities.PlanoAuxiliar.create({ nome: newNome.trim(), marca: newMarca || null, ativo: true });
  ...
  const handleToggle = async (plano) => {
    await api.entities.PlanoAuxiliar.update(plano.id, { ativo: !plano.ativo });
    queryClient.invalidateQueries({ queryKey: ["planos-auxiliares"] });
  };
  ...
  const handleEdit = async (id) => {
    ...
    await api.entities.PlanoAuxiliar.update(id, { nome: editNome.trim(), marca: editMarca || null });
    queryClient.invalidateQueries({ queryKey: ["planos-auxiliares"] });
    setEditId(null);
