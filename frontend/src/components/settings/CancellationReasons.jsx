...
export default function CancellationReasons() {
  ...
  const { data: reasons = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.CancellationReason.list("-created_date", 200),
  });

  const createMutation = useMutation({
    mutationFn: (nome) => api.entities.CancellationReason.create({ nome, ativo: true }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, nome }) => api.entities.CancellationReason.update(id, { nome }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cancellation-reasons"] });
      setEditingId(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.entities.CancellationReason.delete(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cancellation-reasons"] });
