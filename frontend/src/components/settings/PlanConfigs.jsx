...
function EditRow({ plan, onSave, onCancel }) {
  ...
  const carregarProdutos = async () => {
    setLoadingProd(true);
    try {
      const res = await api.functions.invoke('vindiIntegration', { action: 'listar_produtos' });
...
export default function PlanConfigs() {
  ...
  const { data: plans = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.PlanConfig.list("-created_date", 500),
  });

  const saveMutation = useMutation({
    mutationFn: (form) => form.id
      ? api.entities.PlanConfig.update(form.id, form)
      : api.entities.PlanConfig.create(form),
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (plan) => api.entities.PlanConfig.update(plan.id, { ativo: plan.ativo === false ? true : false }),
    onSuccess: (_, plan) => {
      qc.invalidateQueries({ queryKey: ["plan-configs"] });
