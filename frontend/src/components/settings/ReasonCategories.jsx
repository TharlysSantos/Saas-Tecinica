...
export default function ReasonCategories() {
  ...
  const { data: categories = [], isLoading: loadingCats } = useQuery({
    ...
    queryFn: () => api.entities.ReasonCategory.list("-created_date", 200),
  });

  const { data: reasons = [] } = useQuery({
    queryKey: ["cancellation-reasons"],
    queryFn: () => api.entities.CancellationReason.list("nome", 200),
  });

  const createMutation = useMutation({
    mutationFn: () => api.entities.ReasonCategory.create({ nome: newName.trim(), motivos_ids: newMotivos, ativo: true }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id }) => api.entities.ReasonCategory.update(id, { nome: editingName, motivos_ids: editingMotivos }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["reason-categories"] });
      setEditingId(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.entities.ReasonCategory.delete(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["reason-categories"] });
