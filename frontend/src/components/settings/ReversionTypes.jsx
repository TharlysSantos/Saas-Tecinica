...
export default function ReversionTypes() {
  ...
  const { data: revTypes = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.ReversionType.list("-created_date", 200),
  });

  const { data: categories = [] } = useQuery({
    queryKey: ["reason-categories"],
    queryFn: () => api.entities.ReasonCategory.list("nome", 200),
  });

  const createMutation = useMutation({
    mutationFn: () => api.entities.ReversionType.create({ nome: newName.trim(), categorias_ids: newCats, ativo: true }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id }) => api.entities.ReversionType.update(id, { nome: editingName, categorias_ids: editingCats }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["reversion-types"] });
      setEditingId(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.entities.ReversionType.delete(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["reversion-types"] });
