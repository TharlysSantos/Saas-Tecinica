...
export default function StepConfigs() {
  ...
  const { data: configs = [], isLoading } = useQuery({
    ...
    queryFn: () => api.entities.StepConfig.list("etapa", 20),
  });
  ...
  const getOrCreate = async (etapa) => {
    ...
    const created = await api.entities.StepConfig.create({ etapa, grupos: [], analistas: [] });
  ...
  const updateField = async (etapa, field, value) => {
    ...
    await api.entities.StepConfig.update(record.id, { [field]: value });
    queryClient.invalidateQueries({ queryKey: ["step-configs"] });
    setSaving(null);
    toast.success("Salvo!");
  };
